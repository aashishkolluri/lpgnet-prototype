...
def get_avg_dvs_nor_test(
    ...
    if arch is None:
        ...
        dvs_tor = torch.from_numpy(np.array([dvs[j] for j in dvs])).type(torch.float32)
        ...
        for seed in seeds:
            ...
            if "twitch" in dataset.value:
                ...
                data_loader = LoadData(
                    Dataset.TwitchES, dp=dp, test_dataset=test_dataset, rng=rng
                )
            else:
                data_loader = LoadData(
                    dataset, dp=dp, test_dataset=test_dataset, rng=rng
                )
            features = data_loader.features
            labels = data_loader.labels
            ...
            t_str = (
                f"{dataset.name}"
                if not "twitch" in dataset.value
                else f"TwitchES_{test_dataset.name}"
            )
            if arch == Architecture.MMLP:
                if not os.path.isfile(
                    dirpath
                    + "/"
                    + f"arch_mmlp_{nl}_{1}-dataset_{t_str}-seed_{seed}-lr_{lr}-hidden_size_{hs}-num_hidden_{nh}-dropout_{dropout}-eps_{eps}/arch_mmlp_{nl}_{1}-dataset_{t_str}-seed_{seed}-lr_{lr}-hidden_size_{hs}-num_hidden_{nh}-dropout_{dropout}-eps_{eps}.pth"
                ):
                    print(
                        "Not found: ",
                        dirpath
                        + "/"
                        + f"arch_mmlp_{nl}_{1}-dataset_{t_str}-seed_{seed}-lr_{lr}-hidden_size_{hs}-num_hidden_{nh}-dropout_{dropout}-eps_{eps}/arch_mmlp_{nl}_{1}-dataset_{t_str}-seed_{seed}-lr_{lr}-hidden_size_{hs}-num_hidden_{nh}-lr_{lr}-eps_{eps}.pth",
                    )
                    continue
                model_paths = ",".join(
                    [
                        f"arch_mmlp_{nl}_{j}-dataset_{t_str}-seed_{seed}-lr_{lr}-hidden_size_{hs}-num_hidden_{nh}-dropout_{dropout}-eps_{eps}/arch_mmlp_{nl}_{j}-dataset_{t_str}-seed_{seed}-lr_{lr}-hidden_size_{hs}-num_hidden_{nh}-dropout_{dropout}-eps_{eps}.pth"
                ...
                if not os.path.isfile(
                    dirpath
                    + "/"
                    + f"arch_{arch_name}-dataset_{t_str}-seed_{seed}-lr_{lr}-hidden_size_{hs}-num_hidden_{nh}-dropout_{dropout}-eps_{eps}/arch_{arch_name}-dataset_{t_str}-seed_{seed}-lr_{lr}-hidden_size_{hs}-num_hidden_{nh}-dropout_{dropout}-eps_{eps}.pth"
                ):
                    print(
                        "Not found: ",
                        dirpath
            ...
            # get degree vectors
            dvs = getCommunityCountsMP(
                orig_adj_np, pred_labels, num_classes, rng, eps=eps
            )
            dvs_tor = torch.from_numpy(np.array([dvs[j] for j in dvs])).type(
                torch.float32
            )
            avg_dvs_nor_per_seed.append(
                get_avg_nor_given_labels(
                    dvs_tor[test_nodes],
                    pred_labels[test_nodes],
                    labels.cpu().numpy()[test_nodes],
                )
            )

        return [
            np.stack(
                [avg_dvs_nor_per_seed[k][j] for k in range(len(avg_dvs_nor_per_seed))]
            ).mean(axis=0)
            for j in range(len(avg_dvs_nor_per_seed[0]))
        ]


def get_per_class_stats(
    dataset,
    eps,
    outdir,
    arch=None,
    ...
    if arch is None:
        ...
        np_labels = labels.cpu().numpy()
        label_counts = [
            len(np.where(np_labels == label)[0])
        ...
        for seed in seeds:
            ...
            for label in range(np.max(np_labels) + 1):
                ...
                if len(np.where(pred_labels == label)[0]) == 0:
                    ...
                    prec = len(np.where(pred_labels[n_l] == label)[0]) / len(
        ...
        return [
            np.stack([res_per_seed[k][j] for k in range(len(res_per_seed))]).mean(
                axis=0
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--results_dir",
        type=str,
        help="Folder to parse with all the result files (csv files)",
        required=True,
    )
    parser.add_argument(
        "--outdir",
        type=str,
        help="Folder to paste all the tex files",
        required=True,
        default="../plots/",
    )
    parser.add_argument(
        "--best_config_file",
        type=str,
        help="Pkl file with bets hyperparameters",
        default=None,
    )
    parser.add_argument(
        "--only_per_class",
        action="store_true",
        help="True for only computing stats per class",
        default=False,
    )
    parser.add_argument(
        "--dataset",
    ...
    for config in configs:
        ...
        if "twitch" in dataset.value or "flickr" in dataset.value:
            is_inductive = True
        lr, nh, hs, dr = get_lr_nh_hs_dr(
            dataset, arch, args.best_config_file, nl=nl
        )  # Currently hardcoded. Please change
        if (eps > 0 and arch == Architecture.MLP) or (
            nl > 1 and arch == Architecture.MLP
        ):
            continue
        ...
        if not args.only_per_class:
            avg_dvs_nor = get_avg_dvs_nor_test(
                dataset,
                eps,
                args.results_dir,
                arch=arch,
            ...
            if arch is None:
                filename = os.path.join(
                    args.outdir, f"dv_avg_norm_1_test_{dataset.name}_orig.png"
                )
            else:
                filename = os.path.join(
                    args.outdir,
                    f"dv_avg_norm_1_test_{dataset.name}_arch_{arch.name}_eps_{eps}.png",
                )
            if arch == Architecture.MMLP:
                filename = os.path.join(
                    args.outdir,
                    f"dv_avg_norm_1_test_{dataset.name}_arch_{arch.name}_nl_{nl}_eps_{eps}.png",
                )
            print(f"Plotting in file {filename}")
            fmt_str = "ind," + ",".join([str(x) for x in range(len(max_avg_dvs_nor))])
            np.savetxt(
                filename.rstrip(".png") + ".csv",
                np.array([list(range(len(max_avg_dvs_nor)))] + list(max_avg_dvs_nor)).T,
                header=fmt_str,
                delimiter=",",
                comments="",
            )
            np.savetxt(
                filename.rstrip(".png") + "_avg_.csv",
                np.array([list(range(len(avg_dvs_nor)))] + list(avg_dvs_nor)).T,
                header=fmt_str,
                delimiter=",",
                comments="",
            )
            ...
            avg_per_class_res = get_per_class_stats(
                dataset,
                eps,
                args.results_dir,
                arch=arch,
            ...
            if arch == None:
                ...
                np.savetxt(
                    filename.rstrip(".png") + "_nlabels_per_class_.csv",
                    np.array(
                        [list(range(len(avg_per_class_res)))]
                        + [list(avg_per_class_res)]
                    ).T,
                    header=fmt_str,
                    delimiter=",",
                    comments="",
                ...
                fmt_str = "ind," + ",".join(
                    [str(x) for x in range(len(avg_per_class_res))]
                )
                # print(avg_per_class_res)
                print(
                    np.array(
                        [list(range(len(avg_per_class_res[0])))]
                        + list(avg_per_class_res)
                    ).T
                )
                np.savetxt(
                    filename.rstrip(".png") + "_stats_per_class_.csv",
                    np.array(
                        [list(range(len(avg_per_class_res[0])))]
                        + list(avg_per_class_res)
                    ).T,
